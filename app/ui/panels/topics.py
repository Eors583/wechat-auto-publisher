...
def build_topic_center(
    ...
    inner_tabs = ui.tabs().classes("workspace-tabs w-full").props(
...
def _build_hot_topics(
    ...
    with ui.row().classes("w-full items-end gap-3"):
        source_filter = ui.select(
            ...
            label="选题来源（可多选）",
            multiple=True,
        ).classes("col").props("outlined stack-label use-chips clearable")
        days = ui.select({1: "今天", 3: "最近3天", 7: "最近7天", 30: "最近30天"}, value=7, label="日期范围").classes("w-40").props("outlined stack-label")
        keyword = ui.input(
            "热点关键词",
            placeholder="例如：人工智能、组织变革、项目管理",
        ).classes("w-80").props("outlined stack-label clearable")
        unused = ui.switch("只看未使用", value=False)
    with ui.row().classes("w-full items-center gap-2 q-mb-md"):
        refresh_btn = ui.button("刷新所选来源", icon="refresh").props("unelevated color=teal-9 no-caps")
        query_btn = ui.button("搜索多来源热点", icon="search").props("outline color=teal-9 no-caps")
        manual_btn = ui.button("添加手动选题", icon="add").props("flat color=teal-9 no-caps")
    ui.label(
    ...
    def render() -> None:
        ...
        with result_host:
            ...
            ui.label(
                f'{mode_labels.get(mode, "选题")} · 共 {len(items)} 条'
            ).classes("muted")
            if not items:
                ui.label("没有符合条件的选题。可刷新来源或添加手动选题。").classes("muted")
                return
            for item in visible_items:
                with ui.element("div").classes("card w-full"):
                    with ui.row().classes("w-full items-start justify-between"):
                        with ui.column().classes("gap-1").style("flex:1;min-width:0"):
                            ui.label(str(item["title"])).classes("text-subtitle1 text-weight-bold")
                            ...
                            ui.label(
                                " · ".join(
                                    ...
                                    for value in (
                                        ...
                                        published,
                                        SOURCE_TYPES.get(str(item.get("source_type") or ""), ""),
                                    )
                                    if value
                                )
                            ).classes("muted")
                            if item.get("summary"):
                                ui.label(str(item["summary"])[:180]).classes("text-body2")
                        ui.badge("已使用" if item.get("used") else "未使用").props(
                            "color=grey-6" if item.get("used") else "color=orange-7"
                        )
                    with ui.row().classes("items-center gap-2 q-mt-sm"):
                        ...
                        if item.get("url"):
                            ui.link("查看原文", str(item["url"]), new_tab=True).classes("text-teal-9")
                        ui.button(
                            "取消收藏" if item.get("favorite") else "收藏",
                            icon="star" if item.get("favorite") else "star_border",
            ...
            if len(items) > len(visible_items):
                ui.button(
                    f"加载更多选题（剩余 {len(items) - len(visible_items)} 条）",
                    icon="expand_more",
                    on_click=lambda: (
                        result_runtime.__setitem__(
                            "visible_limit",
                            int(result_runtime["visible_limit"]) + 30,
                        ),
                        render(),
                    ),
...
def _build_followed_accounts(
    ...
    with ui.row().classes("w-full items-center justify-between"):
        with ui.column().classes("gap-0"):
            ...
            ui.label("非自有公众号只通过已配置的公众号后台搜索获取微信原文，不再调用搜狗或百度。").classes("muted")
                    ...
                    with ui.row().classes("items-center gap-2 q-mt-sm"):
                        refresh_btn = ui.button(
                            ...
                            if str(account.get("fetch_method") or "") == "official"
                                ...
                                "通过公众号后台获取文章"
                                if str(account.get("fetch_method") or "") == "backend_search"
                                else "获取近期公开文章"
                            ),
                        ...
                        async def refresh_one(
                            ...
                            loading_text = (
                                "正在通过公众号后台搜索…"
                                if str(row.get("fetch_method") or "") == "backend_search"
                                else "正在处理公众号文章…"
                            )
                            ...
                            try:
                                ...
                                ui.notify(f'发现并同步 {report["added"]} 篇文章', type="positive")
                                should_open = True
                            except Exception as exc:  # noqa: BLE001
